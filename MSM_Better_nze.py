...
# import 
techList = pd.read_excel(r'.\model_minerals\Inputs\nze\techListXls.xlsx') #Technology List (oil, gas etc...)
mineralList = pd.read_excel(r'.\model_minerals\Inputs\nze\mineralListXls.xlsx') #Mineral List of raw minerals being used
energyScenario = pd.read_excel(r'.\model_minerals\Inputs\nze\energyScenerio_IEABeyond2Degree.xlsx') #Energy Secenario being tested
lifetime = pd.read_excel(r'.\model_minerals\Inputs\nze\lifetime.xlsx') #Lifetime of Each technology MUST MATCH
techShares = pd.read_excel(r'.\model_minerals\Inputs\nze\techShares.xlsx', sheet_name = 0) #reads in the sub-tech breakdown - indexed to first tab in the sheet
currentProd = pd.read_excel(r'.\model_minerals\Inputs\nze\currentMineralProduction.xlsx') #reads in current mineral production
techIntensity = pd.read_excel(r'.\model_minerals\Inputs\nze\currentTechIntensity.xlsx') #reads in latest Tech Intensity
recycleRates = pd.read_excel(r'.\model_minerals\Inputs\nze\recycleRates.xlsx') # reads in recycle rates for each of the minerals
mineralMetalConvert = pd.read_excel(r'.\model_minerals\Inputs\nze\metalMineralConvert.xlsx') #read in metal to mineral conversion
mineralPrice = pd.read_excel(r'.\model_minerals\Inputs\nze\mineralPrice.xlsx') #read in mineralPrice
...
for tech in range(0,numberOfTech):
  ...
  deltaES_tech = deltaES[:,tech] #selects column vector of stock change for the technology
  ...
  inflow = np.matmul(vInverse,deltaES_tech) #multiples vInverse and DeltaES_tech together
...
for i in range(numberOfTech):
  ...
  if subTechArray[i][1] == 1:
    ...
    tempSubTechVar = pd.read_excel(r'.\model_minerals\Inputs\nze\techShares.xlsx', sheet_name = subTechArray[i][0])
...
for i in range(0,numberOfTech):
    ...
    for m in range(0,numberOfYears):
      for n in range(0,len(mineralList)):
        B[m][n] = jin[m] * k[n] #creating matFlow one-bye one matrix by taking jin at year,mineral and multiply by techIntensity for the given mineral
        Bout[m][n] = -1 * jout[m] * k[n] #creating matFlowOut one-by-one (same as row above)
...
matFlowInFromRecycle = np.zeros([numberOfYears,len(mineralList),numberOfTech], dtype=float) #setting at 3x3 array
...
for i in range(0,numberOfYears):
  for j in range(0,len(mineralList)):
    ...
    for k in range(0,numberOfTech):
      matFlowInFromRecycle[i][j][k] = currentRecycleRate * matFlowOutFinal[i][j][k]
...
for i in range(0,numberOfYears):
  for j in range(0,len(mineralList)):
    for k in range(0,numberOfTech):
      matFlowInVirgin[i][j][k] = matFlowIn[i][j][k] - matFlowInFromRecycle[i][j][k]
      
totalMatFlowInFromRecycle = np.zeros([numberOfYears,len(mineralList)], dtype=float) #setting at 3x3 array
totalMatFlowInVirgin = np.zeros([numberOfYears,len(mineralList)], dtype=float) #setting at 3x3 array  
...
for i in range(0,len(mineralList)):
  for j in range(0,numberOfTech):
    ...
    for k in range(0,numberOfYears):
      ...
      h1 = h1 + matFlowInVirgin[k][i][j] #notice change in order k,i,j - counter k is the years we are summing across
...
virginMarketSizeLow = np.zeros([numberOfYears, len(mineralList),numberOfTech],dtype=float)
virginMarketSizeMed = np.zeros([numberOfYears, len(mineralList),numberOfTech],dtype=float)
virginMarketSizeHigh = np.zeros([numberOfYears, len(mineralList),numberOfTech],dtype=float)

#calculate the Virgin Market Size 
for i in range(0,numberOfYears):
  for j in range(0,len(mineralList)):
    for k in range(0,numberOfTech):
        virginMarketSizeLow[i][j][k] = matFlowInVirgin[i][j][k] * mineralPriceArray[j][0]
        virginMarketSizeMed[i][j][k] = matFlowInVirgin[i][j][k] * mineralPriceArray[j][1]
        virginMarketSizeHigh[i][j][k] = matFlowInVirgin[i][j][k] * mineralPriceArray[j][2]
...
for i in range(0,len(mineralList)):
  for j in range(0,numberOfTech):
    ...
    for k in range(0,numberOfYears):
      ...
      h1 = h1 + virginMarketSizeLow[k][i][j] #notice change in order k,i,j - counter k is the years we are summing across
...
fp = open("./model_minerals/outputs/cleanIeaPowerBi.csv","w") #setting a file pointer
...
fp = open("./model_minerals/outputs/cleanMarketSize.csv","w") #setting a file pointer
...
fp = open("./model_minerals/outputs/cleanRecyclying.csv","w") #setting a file pointer
...
fp = open("./model_minerals/outputs/cleanDemand.csv","w") #setting a file pointer
fp.write("Year,Material,Tons\n")
...
fp = open("./model_minerals/outputs/refreshTime.csv","w")
fp.write("Date-Time\n")
fp.write(timestampStr)
fp.close()

print("IEA NZE Scenerio Done")
